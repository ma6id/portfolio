...
import { PROJECT_COLORS, PROJECT_TEXT_COLORS } from "../lib/projectColors";
...
const getProjectCardStyle = (projectName: string): CSSProperties => {
  const base = PROJECT_COLORS[projectName] ?? "#22d3ee";
    ...
    <section id="projets" className="py-24 border-t border-mist-500/10">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mainVisible.map((p, i) => {
            const accent = PROJECT_COLORS[p.name] ?? "#22d3ee";
            const textAccent = PROJECT_TEXT_COLORS[p.name] ?? "#7dd3fc";

            return (
              <Reveal key={p.slug} delay={i * 90}>
                <button
                  onClick={() => setSelected(p)}
                  className="text-left w-full glass glass-hover has-accent rounded-2xl overflow-hidden h-full flex flex-col focus-ring"
                  style={getProjectCardStyle(p.name)}
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="flex items-center gap-4 mt-5 pt-4 border-t border-mist-500/10">
                      ...
                      <span className="ml-auto text-xs font-mono" style={{ color: textAccent }}>
                        Voir le projet →
                      </span>
                    </div>
                  </div>
          ...
          <div className="mt-14">
            ...
            <Reveal delay={80}>
              <button
                ...
                className="text-left w-full max-w-lg glass glass-hover has-accent rounded-xl overflow-hidden flex items-center gap-4 p-4 focus-ring"
                style={getProjectCardStyle(miniProject.name)}
              >
                ...
                <span
                  className="ml-auto text-xs font-mono shrink-0 pr-1"
                  style={{ color: PROJECT_TEXT_COLORS[miniProject.name] ?? "#7dd3fc" }}
                >
                  Voir →
                </span>
              </button>
